...
export default function CodepenReactify() {
  ...
  useLayoutEffect(() => {
    ...
    const colors = ['#ffdfba', '#baffc9', '#ccc', '#baffc9'];
  ...
  return (
    <>
      <div className='flex justify-center items-center h-[50vh] overflow-x-hidden'>
        <button className='prev'>prev</button>

        <button className='next'>next</button>

        <button className='toggle' onClick={toggleOverflow}>
          toggle overflow
        </button>
      ...
      <ul className='bg-gray-800 p-4'>
        <li>
          overflow -{' '}
          <span className={`${overflow ? 'text-green-300' : 'text-red-200'}`}>
            {JSON.stringify(overflow)}
          </span>
        </li>

        <li>
          active item - <span>{JSON.stringify(activeIndex + 1)}</span>
        </li>
      </ul>
    </>
  );
